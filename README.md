...
| GET | `/api/cabinets/next-number` | 获取下一个可用编号（智能填补空缺） | ❌ |
...
### 最近更新 (v1.3.0)
- 🔍 搜索扩展：支持按品牌标签名搜索（如搜"丰巢"），搜索结果展示编号+品牌+名称
- 🔢 编号智能分配：删除 C-02 后新建自动分配 C-02（填补空缺），而非跳到 C-13
- 🔗 编号名称一致：创建柜机时 name 自动从 number 提取数字（C-02→2号柜）
- 🆕 新增 API：GET /api/cabinets/next-number 获取下一个可用编号
